...
def _decode_unicode_escapes(text: str) -> str:
    """JSONLoader가 만들어 내는 \\uXXXX 이스케이프를 한글로 복원.

    JSONLoader(text_content=False) 는 page_content 를 ASCII-safe 형태로 저장하기 때문에
    한글이 \\uXXXX 로 박혀 있다. 검색·시각화에서 의미 있게 다루려면 한 번 디코딩해 둬야 한다.
    """
    if "\\u" not in text:
        return text
    try:
        parsed = json.loads(text)  # {"k": "\uXXXX..."} → {"k": "한글..."}
...
def _normalize_docs(docs: list[Document]) -> list[Document]:
    for d in docs:
        d.page_content = _decode_unicode_escapes(d.page_content)
    return docs


def load_one(spec: SourceSpec) -> list[Document]:
    """5종 로더 분기를 하나의 함수로 통일. JSON 계열은 한글 복원 후 반환."""
    if spec.loader_kind == "text":
        return _build_text_loader(spec).load()
    if spec.loader_kind == "json":
        return _normalize_docs(_build_json_loader(spec, json_lines=False).load())
    if spec.loader_kind == "jsonl":
        return _normalize_docs(_build_json_loader(spec, json_lines=True).load())
...
def _word_set(text: str) -> set[str]:
    """간이 토크나이저 — 한글·영문 단어 단위로 분리."""
    import re
    tokens = re.findall(r"[A-Za-z0-9]+|[가-힣]+", text)
    return {t.lower() for t in tokens if len(t) >= 2}


def _bm25_lite_score(query: str, doc: str) -> float:
    """단어 일치 기반 간이 관련도 점수 (실제 RAG는 임베딩 사용 — 데모용)."""
    q_set = _word_set(query)
    d_set = _word_set(doc)
    if not q_set or not d_set:
        return 0.0
    overlap = len(q_set & d_set)
    return overlap / (len(q_set) ** 0.5 * (1 + 0.1 * len(d_set)))


def _wrap_korean(text: str, width: int = 70) -> list[str]:
    """한글이 섞인 문자열을 고정 폭으로 끊어 리스트 반환."""
    import textwrap
    text = " ".join(text.split())  # 다중 공백 정리
    return textwrap.wrap(
        text, width=width, break_long_words=True, break_on_hyphens=False
    )


def visualize_topk_retrieval() -> Path:
    """3개 샘플 질문 → 해당 컬렉션에서 Top-2 청크 검색 결과 카드.

    텍스트를 미리 wrap하여 박스 높이를 줄 수에 맞춰 동적으로 계산한다.
    """
    samples: list[tuple[str, str]] = [
        ("모든전자의 슬로건과 핵심 가치를 알려줘.", "company_profile"),
        ("초고밀도 전력 모듈의 효율은 몇 %야?", "products_services"),
        ("디지털 직무 채용에는 코딩 과제가 있어?", "recruitment_process_guide"),
    ]
    spec_by_key = {s.key: s for s in SOURCES}
    max_lines_per_chunk = 4
    wrap_width = 60

    # 각 카드의 내용을 미리 계산 (wrap 결과 + 점수)
    cards: list[tuple[str, str, list[tuple[float, list[str]]]]] = []
    for query, collection_key in samples:
        spec = spec_by_key[collection_key]
        chunks = split_one(spec, load_one(spec))
        scored = sorted(
            ((_bm25_lite_score(query, c.page_content), c) for c in chunks),
            key=lambda x: -x[0],
        )[:2]
        results: list[tuple[float, list[str]]] = []
        for score, doc in scored:
            wrapped_full = _wrap_korean(doc.page_content, wrap_width)
            wrapped = wrapped_full[:max_lines_per_chunk]
            if len(wrapped_full) > max_lines_per_chunk:
                wrapped[-1] = wrapped[-1][:wrap_width - 3] + "..."
            results.append((score, wrapped))
        cards.append((query, collection_key, results))

    # 레이아웃 상수 (단위: figure 좌표)
    LINE_H = 0.55
    QHEAD_H = 1.0
    PAD_TOP, PAD_BOT = 0.30, 0.30
    HEADER_H = 0.65
    GAP_RESULT = 0.20
    GAP_CARD = 0.65

    # 카드별 높이 계산
    def card_height(results: list[tuple[float, list[str]]]) -> float:
        h = QHEAD_H + GAP_RESULT
        for _, lines in results:
            h += PAD_TOP + HEADER_H + len(lines) * LINE_H + PAD_BOT + GAP_RESULT
        return h

    total_h = sum(card_height(c[2]) for c in cards) + GAP_CARD * (len(cards) - 1) + 0.4
    fig_h = max(total_h * 0.42, 12.0)  # 인치 단위로 환산
    ...
    cur_y = total_h - 0.2
    for query, collection_key, results in cards:
        # 질문 헤더
        ax.add_patch(plt.Rectangle(
            (0.1, cur_y - QHEAD_H), 9.8, QHEAD_H,
            facecolor="#FFE0B2", edgecolor="#5D4037", linewidth=1.0,
        ))
        ax.text(0.28, cur_y - QHEAD_H / 2, f"Q. {query}",
                fontsize=11.5, fontweight="bold", va="center")
        ax.text(9.72, cur_y - QHEAD_H / 2, f"→ collection: {collection_key}",
                fontsize=9, ha="right", va="center", color="#555")
        cur_y -= QHEAD_H + GAP_RESULT

        # 결과 박스 2개
        for rank, (score, lines) in enumerate(results):
            box_h = PAD_TOP + HEADER_H + len(lines) * LINE_H + PAD_BOT
            ax.add_patch(plt.Rectangle(
                (0.1, cur_y - box_h), 9.8, box_h,
                facecolor="#E8F5E9", edgecolor="#2E7D32", linewidth=0.9,
            ))
            ax.text(0.28, cur_y - PAD_TOP - HEADER_H / 2,
                    f"Top-{rank + 1}    relevance ≈ {score:.3f}",
                    fontsize=10.5, fontweight="bold", color="#2E7D32", va="center")
            for i, line in enumerate(lines):
                y_line = cur_y - PAD_TOP - HEADER_H - i * LINE_H - LINE_H / 2
                ax.text(0.28, y_line, line, fontsize=9.5, va="center", color="#212121")
            cur_y -= box_h + GAP_RESULT

        cur_y -= GAP_CARD - GAP_RESULT  # 카드 간격

    fig.suptitle(
        "샘플 질문 → Top-2 청크 검색 결과 (단어 일치 기반 데모)",
        fontsize=14, fontweight="bold", y=0.995,
    )
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    out = RESULTS_DIR / "fig_05_topk_retrieval.png"
    fig.savefig(out, dpi=140)
    plt.close(fig)
    return out


def visualize_collection_preview() -> Path:
    """5개 컬렉션 각각의 내용 미리보기 카드.

    텍스트를 미리 wrap하여 박스 높이를 줄 수에 맞춰 동적으로 계산한다.
    """
    palette = ["#E3F2FD", "#FFF3E0", "#F3E5F5", "#E8F5E9", "#FCE4EC"]
    border = ["#1565C0", "#E65100", "#6A1B9A", "#2E7D32", "#AD1457"]
    max_preview_lines = 3
    wrap_width = 65

    cards: list[tuple[SourceSpec, int, int, list[str], str, str]] = []
    for spec, bg, bd in zip(SOURCES, palette, border):
        chunks = split_one(spec, load_one(spec))
        avg_len = int(sum(len(c.page_content) for c in chunks) / max(len(chunks), 1))
        wrapped_full = _wrap_korean(chunks[0].page_content, wrap_width)
        wrapped = wrapped_full[:max_preview_lines]
        if len(wrapped_full) > max_preview_lines:
            wrapped[-1] = wrapped[-1][:wrap_width - 3] + "..."
        cards.append((spec, len(chunks), avg_len, wrapped, bg, bd))

    LINE_H = 0.55
    HEAD_H = 0.75
    SUB_H = 0.65
    PAD_TOP, PAD_BOT = 0.30, 0.35
    GAP = 0.50

    def card_height(lines: list[str]) -> float:
        return PAD_TOP + HEAD_H + SUB_H + len(lines) * LINE_H + PAD_BOT

    total_h = sum(card_height(c[3]) for c in cards) + GAP * (len(cards) - 1) + 0.4
    fig_h = max(total_h * 0.42, 13.0)
    fig, ax = plt.subplots(figsize=(13, fig_h))
    ax.set_xlim(0, 10)
    ax.set_ylim(0, total_h)
    ax.axis("off")
    ...
    cur_y = total_h - 0.2
    for spec, n_chunks, avg_len, lines, bg, bd in cards:
        h = card_height(lines)
        ax.add_patch(plt.Rectangle(
            (0.08, cur_y - h), 9.84, h,
            facecolor=bg, edgecolor=bd, linewidth=1.4,
        ))
        ax.text(0.28, cur_y - PAD_TOP - HEAD_H / 2,
                f"■  {spec.label}  ({spec.key})",
                fontsize=12.5, fontweight="bold", va="center", color=bd)
        ax.text(9.72, cur_y - PAD_TOP - HEAD_H / 2,
                f"chunks={n_chunks}  ·  avg_len≈{avg_len}자  ·  loader={spec.loader_kind}",
                fontsize=9, ha="right", va="center", color="#555")
        ax.text(0.28, cur_y - PAD_TOP - HEAD_H - SUB_H / 2,
                "▶ 첫 번째 청크 미리보기",
                fontsize=10, fontweight="bold", va="center", color="#333")
        for i, line in enumerate(lines):
            y_line = cur_y - PAD_TOP - HEAD_H - SUB_H - i * LINE_H - LINE_H / 2
            ax.text(0.28, y_line, line, fontsize=9.5, va="center", color="#212121")
        cur_y -= h + GAP

    fig.suptitle("5개 컬렉션 내용 미리보기",
                 fontsize=14, fontweight="bold", y=0.995)
    fig.tight_layout(rect=(0, 0, 1, 0.97))
    out = RESULTS_DIR / "fig_06_collection_preview.png"
    fig.savefig(out, dpi=140)
    plt.close(fig)
    return out
