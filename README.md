...
> 채용 PDF는 134 KB로 가장 무거운데 청크는 3개에 그치고(이미지·표가 많아 텍스트 비중이 적음), CSV는 0.5 KB지만 한 줄당 한 Document라 6개로 늘어남. **파일 크기와 검색 단위(청크 수)는 일치하지 않는다**는 점이 한눈에 보.
...
| **임베딩과 LLM은 다른 서버로** | 처음에는 한 프록시 URL로 두 호출을 다 했는데, 임베딩이 LLM 응답을 막는 병목이 됨. URL을 두 개로 분리하자 두 호출이 독립적으로 처리됨. → 운영 환경을 가정한다면 처음부터 **임베딩 서버와 LLM 서버를 분리**해 두는 것이 확장성·교체 측면에서 유리함. |

---

## 💡 회고록 (Retrospective)

처음에 RAG는 솔직히 잘 와닿지 않았습니다. "검색 + 생성"이라는 한 줄 설명은 굉장히 많이 본 것 같은데, 막상 코드로는 어디서부터 손대야 할지 그림이 안 그려졌습니다. 그래서 이번에는 일부러 "튜토리얼 따라 해 보기"가 아니라, 다섯 종류의 문서를 직접 다 흘려 보고 부딪히는 식으로 접근해 봤습니다. 막상 해 보니 RAG는 문서를 자르고, 비슷한 조각을 찾고, LLM에게 같이 넘긴다. 그게 전부였습니다. 단순한 만큼 어디를 잘못 손대면 검색이 무너지는지가 더 또렷하게 보였습니다.

가장 크게 깨진 부분은 JSON이었습니다. 처음엔 "그냥 다 500자로 자르면 되겠지"라고 생각했고 글자 수 기준으로 잘라 임베딩하고 "GaN 모듈 효율"을 물었더니, 엉뚱하게 키 이름만 들어 있는 청크가 위로 올라왔습니다. 한참 들여다보고 나서야 JSON은 키와 값이 한 묶음이어야 의미가 살아 있다는 사실을 깨달았습니다. 글자 수만 기준으로 자르면 키와 값이 따로 떨어진 조각이 생기고, 검색 입장에서는 그게 그냥 무의미한 텍스트와 다를 게 없습니다. 결국 `chunkable` 플래그를 하나 두어서 JSON/JSONL은 자르지 않고 통째로 임베딩하도록 따로 처리했고, 그것만으로 사양 질문의 검색 정확도가 확 살아났습니다. 자료의 형태에 따라 처리 방식이 달라야 한다는 말을, 머리가 아니라 손으로 배운 순간이었습니다.

컬렉션을 어떻게 나눌지는 한참 고민했던 부분입니다. "어차피 임베딩이 비슷한 걸 알아서 찾는데 컬렉션은 하나면 되는 거 아닌가?"라는 생각이 처음에는 강했습니다. 그런데 실제로 한 통에 다 넣어 보니, 채용 관련 질문에 회사 연혁 청크가 떡하니 섞여 들어왔습니다. 잡음이 끼는 정도가 아니라 LLM이 그 잡음을 진짜로 받아서 어색한 답을 만드는 게 더 문제였습니다. 컬렉션을 5개로 쪼개고 검색 함수도 5개로 따로 만들었더니, 챗봇이 먼저 "이 질문은 채용 쪽만 보면 되겠네"라고 판단한 뒤에 검색을 시작하기 때문에, 다른 분야 문서가 결과에 섞여 들어올 일이 아예 사라졌습니다. 검색 품질은 임베딩 모델 같은 걸로만 잡히는 게 아니라, **검색 대상을 처음에 어떻게 나눠 두느냐** 가 그보다 위에 있는 결정이라는 걸 이때 알게 됐습니다. 더불어, 함수 설명(docstring)을 짧게 적어 두니 챗봇이 어떤 함수를 부를지 갈팡질팡하는 일이 잦았는데, "구체적인 제품 사양·스펙 질문에 사용"처럼 언제 부르는지를 분명히 적어 두자 그런 어긋남이 거의 사라졌습니다. 함수 설명이 단순한 사용 안내가 아니라, 챗봇이 어디로 가야 할지를 정해 주는 규칙처럼 작동한다는 점이 의외였습니다.

다음 단계로는 두 가지 방향으로 더 가 보고 싶습니다. 하나는 검색 자체를 더 똑똑하게 만드는 쪽 입니다. 지금은 임베딩이 찾아 준 청크를 그대로 LLM에 넘기고 있는데, 한 번 더 추려서(re-ranking) 가장 핵심적인 조각만 남기거나, 여러 컬렉션의 결과를 섞어서 답을 종합하는 구조까지 가 보면 검색 품질이 한층 안정적으로 잡힐 것 같습니다. 다른 하나는 챗봇 자체를 실제 서비스에 가깝게 다듬는 쪽 입니다. 지금 만든 챗봇은 한 번에 한 질문만 받고 이전 대화를 전혀 기억하지 못하는 데다, 응답도 매번 자유로운 문장으로만 나오기 때문에 회사 시스템에서 그 결과를 자동으로 처리하기에는 한계가 분명합니다. 다음 포트폴리오에서는 대화 기억 과 응답 형식 정해 두기 를 함께 다뤄 보면서, 단순 시연을 넘어 실제 서비스에서도 쓸 수 있는 챗봇으로 한 걸음 더 나아가 볼 생각입니다. 이번 RAG 파이프라인이 그 출발점이 되어 줄 것 같습니다.
